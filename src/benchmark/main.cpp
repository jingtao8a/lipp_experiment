...
/*
 * Simple benchmark that runs a mixture of point lookups and inserts on DILI.
 */
#include <fstream>
#include <iostream>
#include <functional>
...
int main(int argc, char* argv[]) {
  ...
  std::sort(keys, keys + init_num_keys);
  ...
  while (true) {
    ...
    if (i > 0) {
      ...
      if (lookup_distribution == "uniform") {
        lookup_keys = get_search_keys(keys, i + std::min(num_inserts_per_batch, total_num_keys - i), num_lookups_per_batch);
      } else if (lookup_distribution == "zipf") {
        lookup_keys = get_search_keys_zipf(keys, i + std::min(num_inserts_per_batch, total_num_keys - i), num_lookups_per_batch);
      } else {
