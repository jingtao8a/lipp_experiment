...
//            for (int _ = 0; _ < 1e7; _ ++) {
    ...
    void insert(const T& key, const P& value) {
        //get segment index
        auto index = ts_->GetSplineSegment(key);
        RT_ASSERT(index >= 0);
...
private:
    ...
    std::unique_ptr<ts::TrieSpline<T>> ts_;
    ...
    void build_tree_bulk_only_once(T* _keys, P* _values, int _size)
    {
        ...
        ts_ = std::make_unique<ts::TrieSpline<T>>(std::move(tsb.Finalize()));
        std::cout << "construct ts end" << std::endl;

        auto n = ts_->spline_points_.size();
        std::cout << "segments size: " << ts_->spline_points_.size() << std::endl;

        std::cout << "build segmentNodes ......" << std::endl;
        segmentNodes.resize(n + 1);
        ...
        auto _s = 0;
        for (int i = 0; i < n; ++i) {
            nextIter = std::lower_bound(beginIter, endIter, ts_->spline_points_[i + 1].x);
            _s = nextIter - beginIter;
            if (_s <= 1) {
                segmentNodes[i] = build_tree_none();
                for (auto _ = 0; _ < _s; ++_) {
                    insert_tree(segmentNodes[i], _keys[index + _], _values[index + _]);
                }
            } else {
        ...
        _s = nextIter - beginIter;
        if (_s <= 1) {
            segmentNodes.back() = build_tree_none();
        } else {
            segmentNodes.back() = build_tree_bulk(_keys + index, _values + index, _s);
        }
        index += _s;
        beginIter = nextIter;
        if (index != _size) {
            ...
            std::cout << "最后一个spline_point x" << ts_->spline_points_.back().x << std::endl;
            std::cout << "倒数第二个spline_point x" << (ts_->spline_points_.end() - 2)->x << std::endl;
        }
        RT_ASSERT(index == _size);
        RT_ASSERT(beginIter == endIter);
        std::cout << "build segmentNodes end" << std::endl;
    }
    /// bulk build, _keys must be sorted in asc order.
